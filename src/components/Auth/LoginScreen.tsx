...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onGoogleSignIn }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-center items-center px-4 py-12 font-sans relative overflow-hidden">
      ...
      <div className="max-w-md w-full glass-panel p-8 rounded-3xl shadow-2xl border border-slate-800/80 space-y-8 relative z-10">

        {/* App Brand Header */}
        <div className="text-center space-y-3">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-slate-900 border border-slate-700 shadow-xl shadow-violet-500/10 mb-2">
          ...
          <h1 className="text-3xl font-black tracking-tight text-white font-display">
            Weight<span className="text-violet-400">Tracker</span> <span className="text-xs uppercase font-extrabold text-violet-400 px-2 py-0.5 rounded bg-violet-950 border border-violet-800 align-middle">PWA</span>
          </h1>
        ...
        {/* Error Notification */}
        {errorMsg && (
          <div className="bg-red-950/60 border border-red-800/60 p-3 rounded-xl text-xs font-semibold text-red-300 text-center">
            {errorMsg}
          </div>
        )}
        ...
        <div className="space-y-4 pt-2">
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-slate-900 hover:bg-slate-800 text-white font-bold text-sm py-3.5 px-5 rounded-2xl border border-slate-700 hover:border-violet-500/50 shadow-xl transition-all disabled:opacity-50 group"
          >
            {isSigningIn ? (
        ...
        <div className="pt-4 border-t border-slate-800/80 grid grid-cols-3 gap-2 text-center">
          <div className="p-2">
            <Scale className="w-4 h-4 text-violet-400 mx-auto mb-1" />
            <span className="text-[10px] font-medium text-slate-400 block">Daily Weigh-Ins</span>
          </div>
          <div className="p-2">
            <Cloud className="w-4 h-4 text-cyan-400 mx-auto mb-1" />
            <span className="text-[10px] font-medium text-slate-400 block">Cloud Sync</span>
          </div>
          <div className="p-2">
            <Users className="w-4 h-4 text-amber-400 mx-auto mb-1" />
            <span className="text-[10px] font-medium text-slate-400 block">Family Household</span>
          </div>
        </div>
