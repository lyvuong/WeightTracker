    ...
    <div className="space-y-5">
      <div className="glass-panel p-5 rounded-3xl flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-lg font-black text-white font-display">People</h1>
        ...
        <button
          onClick={onAddPerson}
          className="flex items-center gap-2 bg-gradient-to-r from-violet-500 to-fuchsia-600 hover:from-violet-400 hover:to-fuchsia-500 text-white font-bold text-xs px-4 py-2.5 rounded-xl shadow-lg shadow-violet-500/20 transition-all"
        >
      ...
      {unprofiled.map(member => (
        <div
          key={member.uid}
          className="glass-card p-4 rounded-2xl border border-amber-800/50 bg-amber-950/20 flex items-center justify-between gap-3 flex-wrap"
        >
          ...
          <button
            ...
            className="text-xs font-bold text-amber-200 bg-amber-900/50 hover:bg-amber-900 border border-amber-700/60 px-3 py-1.5 rounded-xl transition-all whitespace-nowrap"
          >
            Add profile
          </button>
        </div>
      ...
      {people.length === 0 ? (
        <div className="glass-panel p-10 rounded-3xl text-center">
          <p className="text-sm font-bold text-white">No people yet</p>
          <p className="text-xs text-slate-400 mt-1">Add everyone whose weight you want to track.</p>
        </div>
      ) : (
              ...
              <div key={person.id} className="glass-panel p-5 rounded-3xl space-y-4">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-base font-black text-white font-display leading-tight">{person.name}</h3>
                      <p className="text-[11px] text-slate-400">
                  ...
                  <button
                    ...
                    className="p-2 text-slate-400 hover:text-violet-400 bg-slate-800/80 hover:bg-slate-700 rounded-xl border border-slate-700 transition-all"
                ...
                <dl className="space-y-2 text-xs">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <dd className="font-mono font-bold text-slate-200">{formatHeight(person.heightCm, unit)}</dd>
                  </div>
                  <div className="flex items-center justify-between">
                    <dt className="flex items-center gap-1.5 text-slate-400">
