  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <form
        ...
        className="glass-panel w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl border border-slate-800 shadow-2xl bg-slate-900/95 p-6 space-y-5"
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <PersonAvatar person={person} size="md" />
            <div>
              <h2 className="text-lg font-black text-white font-display leading-tight">{person.name}</h2>
        ...
        {queue && queue.total > 1 && (
          <div className="flex items-center gap-1.5">
            {Array.from({ length: queue.total }).map((_, i) => (
              <span
                key={i}
                className={`h-1.5 flex-1 rounded-full ${i <= queue.index ? 'bg-violet-500' : 'bg-slate-700'}`}
              />
            ))}
          </div>
        )}
        ...
        <div className="bg-slate-950/70 border border-slate-800 rounded-2xl p-5 text-center space-y-1">
          <div className="flex items-baseline justify-center gap-2">
            <input
              ...
              className="w-40 bg-transparent text-center text-5xl font-black font-mono text-white placeholder:text-slate-700 focus:outline-none"
            />
            <span className="text-2xl font-bold text-violet-400">{unit}</span>
          </div>
          {lastWeightKg !== null && (
          ...
          {showWhen && (
            <div className="grid grid-cols-2 gap-2">
              <input
                ...
                className="glass-input text-sm text-white px-3 py-2 rounded-xl"
              />
              <input
                type="time"
                value={time}
          ...
          {showMore && (
            <div className="space-y-2">
              <input
                ...
                className="w-full glass-input text-sm text-white px-3 py-2 rounded-xl placeholder:text-slate-600"
              />
              <input
                type="text"
        ...
        <button
          type="submit"
          disabled={!isValid}
          className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-violet-500 to-fuchsia-600 hover:from-violet-400 hover:to-fuchsia-500 text-white font-bold text-sm py-3.5 rounded-2xl shadow-lg shadow-violet-500/20 active:scale-[0.99] transition-all disabled:opacity-40"
        >
